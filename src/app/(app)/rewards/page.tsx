  ...
  return (
    <div className="flex gap-1 text-3xl leading-none">
      {Array.from({ length: total }, (_, i) => (
        <span key={i} className={i < filled ? "text-gold" : "text-navy/15"}>
          ★
        </span>
      ))}
    </div>
...
function ParentStarView({
  ...
  return (
    <section className="flex flex-col gap-3">
      ...
      />
      {earned && (
        <div className="rounded-lg bg-gold/20 p-3 text-center font-bold text-navy">
...
function RecentStars({ stars }: { stars: RecentStar[] }) {
  if (stars.length === 0) {
    return <p className="text-sm text-navy/60">No stars given yet.</p>;
  }
  ...
  return (
    <ul className="flex flex-col gap-1 text-sm">
      {stars.map((star) => (
        <li
          key={star.id}
          className="flex justify-between rounded border border-navy/10 bg-white px-3 py-2"
        >
